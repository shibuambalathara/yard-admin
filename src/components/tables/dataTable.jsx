...
const DataTable = ({ data, columns }) => {
  ...
  return (
    <div className="w-full mx-auto flex flex-col items-center p-2 sm:p-8">
      ...
      <div className="mt-4 ring-1 w-full h-fit ring-gray-300 rounded-lg overflow-auto hidden sm:block">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr className="divide-x divide-gray-500" key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    className="py-3.5 pl-2 pr-1 text-xs sm:text-sm font-semibold text-gray-100 text-left cursor-pointer"
                    key={header.id}
